...
class Accounts {
    ...
    protected function check_unused_username($username) {
        ...
        $username = $this->tData->db->real_escape_string($username);
        $query = $this->tData->db->query("SELECT * FROM `".$this->tData->prefix."_users` WHERE `username`='".$username."'");
    ...
    private function email_registered_user($email, $activation_code) {
        ...
        $query = $this->tData->db->query("SELECT * FROM `".$this->tData->prefix."_settings`");
    ...
    protected function create_registered_user($args) {
        ...
        if ($this->define_username($user_variables['username']) !== true) {
            return $this->register_error("The username you've provided is invalid.");
            ...
            $user_variables['username'] = $this->tData->db->real_escape_string($user_variables['username']);
        }
        ...
        if ($this->define_password($user_variables['password']) != true) {
            ...
            if ($user_variables['password'] != $user_variables['password-repeat']) {
                ...
                $user_variables['password'] = $this->tData->db->real_escape_string(hash('SHA256', $user_variables['password'].$salt));
            }
        }
        ...
        if ($this->define_email($user_variables['email']) != true) {
            ...
            $user_variables['email'] = $this->tData->db->real_escape_string($user_variables['email']);
        }
        ...
        if (strlen($user_variables['first-name']) > 50) {
            ...
            $user_variables['first-name'] = $this->tData->db->real_escape_string($user_variables['first-name']);
        }
        ...
        if (strlen($user_variables['last-name']) > 125) {
            ...
            $user_variables['last-name'] = $this->tData->db->real_escape_string($user_variables['last-name']);
        }
        ...
        // Register the user
        $this->tData->db->autocommit(false);
        $query = $this->tData->db->query("INSERT INTO `".$this->tData->prefix."_users` ".
            ...
            " VALUES ".
            "('".$user_variables['username']."', '".$user_variables['password']."', '".$user_variables['email']."', ".
            "'".$user_variables['first-name']."', '".$user_variables['last-name']."', '".$user_variables['birthday']."', ".
            "'".$user_variables['gender']."', '".$user_variables['admin']."', '".$user_variables['groups']."', ".
            "'0', '".$user_variables['phone']."', 'default-user-picture.png', now(), '0', '".$user_variables['activation-code']."')");
